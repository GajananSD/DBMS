...
def dbms():
    ...
    # Fetch form data
        ...
        if search_type == 'title':
            resultValue = cur.execute("SELECT book.*, PlacedOn.section_id, PlacedOn.row_no FROM book INNER JOIN PlacedOn ON book.book_id = PlacedOn.book_id WHERE book.title LIKE '%" + search_query + "%';")
        elif search_type == 'book_id':
            resultValue = cur.execute("SELECT book.*, PlacedOn.section_id, PlacedOn.row_no FROM book INNER JOIN PlacedOn ON book.book_id = PlacedOn.book_id WHERE book.book_id = %s;", (search_query,))
        elif search_type == 'author_id':
            resultValue = cur.execute("SELECT book.*, PlacedOn.section_id, PlacedOn.row_no FROM book INNER JOIN PlacedOn ON book.book_id = PlacedOn.book_id WHERE book.author_id = %s;", (search_query,))
